...
from fasthtml.oauth import GoogleAppClient, redir_url, GitHubAppClient
...
class FastHTMLAuth:
    ...
    async def oauth_login(self, request, provider, code: str = None):
        ...
        auth_callback_path = f"/auth/oauth/{provider}"
        ...
        if code:
            ...
            if not user_info.get("email") and provider == "github":
                client.info_url = "https://api.github.com/user/emails"
                emails = client.get_info()
                user_info["email"] = emails[0]["email"]
            if User.get_by_email(user_info["email"]):
                return User.get_by_email(user_info["email"])
        ...
        return client.login_link(redir)
